  ...
  .run(['$rootScope', '$timeout', '$modal', '$translate', '$location', '$window', '$http', '$q',
        function($rootScope, $timeout, $modal, $translate, $location, $window, $http, $q) {
            ...
            function fetchModel(modelId) {
                var tenantId  = 1
                if (localStorage.getItem('laputa-tenantId')) {
                  tenantId = JSON.parse(localStorage.getItem('laputa-tenantId')).content
                }

                var token = ''
                if (sessionStorage.getItem('laputa-access_token')){
                  token = JSON.parse(sessionStorage.getItem('laputa-access_token')).content
                }
