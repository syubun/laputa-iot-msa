...
import com.laputa.iot.msg.api.entity.MeetingSession;
import com.laputa.iot.msg.api.enums.EventEnum;
import com.laputa.iot.msg.api.enums.SessionStatusEnum;
...
public class MeetingServiceImpl {
    ...
    @Autowired
    public MeetingServiceImpl(SimpMessagingTemplate simpMessagingTemplate) {
     ...
     */
    public MeetingSession preMeetingProcess(Long hostId,Long peerId,String sessionId,boolean isAccept, Map<String,Object> signalingMap) throws ServiceInternalException {
        if(NumberHelper.isIDValid(hostId) &&NumberHelper.isIDValid(peerId)) {
...
//                if(!UserActiveStatusEnum.getAllOnLineCodeList().contains(userService.getUserActiveStatus(hostId))){
                if(!UserActiveStatusEnum.ACTIVE_VISIBLE.getCode().equals(userService.getUserActiveStatus(hostId))){
                    ...
                    EventDTO<MeetingSession> meetingSessionEventToHost = EventDTO.buildPersonal(EventEnum.EVENT_CALL_FAIL,"自己需保持在线状态",hostId,null);
                ...
                if (redisUtils.hHasKey(MEETING_USER_ACTIVITY,String.valueOf(peerId))) {    // 对方有meeting session，说明对方在忙
                    meetingSession.setStatus(SessionStatusEnum.FAIL.getCode());
                    EventDTO<MeetingSession> meetingSessionEventToHost = EventDTO.buildPersonal(EventEnum.EVENT_CALL_BUSY,hostId,meetingSession);
                    sendPersonalEvent(hostId,meetingSessionEventToHost);
                    ...
                    if(userService.getUserActiveStatus(peerId).equals(UserActiveStatusEnum.ACTIVE_VISIBLE.getCode())||(userService.getUserActiveStatus(peerId).equals(UserActiveStatusEnum.ACTIVE_ONLY_CATEGORY_BOOKMARK.getCode())&& userService.isInPeerBookMark(hostId,peerId))){
                        ...
                        R<List<RelationVO>> result = userService.getUserRelation(peerId,hostId, ContainerEnum.PERSON.getCode(),null);
                        if(result.getCode()!= ResultCode.SUCCESS.getCode()||result.getData()==null||result.getData().size()==0){
                            // 如果获取不到对方关系信息，说明对方不存在或者账号异常
                            meetingSession.setStatus(SessionStatusEnum.FAIL.getCode());
                            EventDTO<MeetingSession> meetingSessionEventToHost = EventDTO.buildPersonal(EventEnum.EVENT_CALL_FAIL,"无法获取对方信息",hostId,meetingSession);
                            sendPersonalEvent(hostId,meetingSessionEventToHost);
                            throw new ServiceInternalException("无法获取对方信息");
                        }

                        redisUtils.hSet(MEETING_USER_ACTIVITY ,String.valueOf(peerId),sessionId,MEETING_USER_ACTIVITY_TIME);
                        redisUtils.hSet(MEETING_USER_ACTIVITY ,String.valueOf(hostId),sessionId,MEETING_USER_ACTIVITY_TIME);
                        meetingSession.setStatus(SessionStatusEnum.IN_CONFIRM.getCode());
                        redisUtils.hSet(MEETING_SESSION,sessionId, meetingSession);
                        EventDTO<MeetingSession> meetingSessionEventToHost = EventDTO.buildPersonal(EventEnum.EVENT_CALL_RING,hostId,meetingSession);
                        sendPersonalEvent(hostId,meetingSessionEventToHost);
                        RelationVO relationVO = result.getData().get(0);
                        meetingSession.setData(relationVO);
                        EventDTO<MeetingSession> meetingSessionEventToPeer = EventDTO.buildPersonal(EventEnum.EVENT_CALL_IN,peerId,meetingSession);
                        sendPersonalEvent(peerId,meetingSessionEventToPeer);
                        return meetingSession;
                    }else{
                        // 如果对方不在线，或者对方设置为只允许Bookmark，就返回Offline消息
                        meetingSession.setStatus(SessionStatusEnum.FAIL.getCode());
                        EventDTO<MeetingSession> meetingSessionEventToHost = EventDTO.buildPersonal(EventEnum.EVENT_CALL_OFFLINE,hostId,meetingSession);
                        sendPersonalEvent(hostId,meetingSessionEventToHost);
                ...
                if(!isAccept){
                    ...
                    EventDTO<MeetingSession> meetingSessionEventToRequester;
                    EventDTO<MeetingSession> meetingSessionEventToPeer;
                    if(!meetingSession.getStatus().equals(SessionStatusEnum.IN_MEETING.getCode())){
                        ...
                        if(hostId.equals(meetingSession.getRequesterId())){
                            meetingSessionEventToRequester = EventDTO.buildPersonal(EventEnum.EVENT_CALL_FAIL,meetingSession.getRequesterId(),null);
                            meetingSessionEventToPeer = EventDTO.buildPersonal(EventEnum.EVENT_CALL_DENY,meetingSession.getPeerId(),null);

                        }else{
                            meetingSessionEventToRequester = EventDTO.buildPersonal(EventEnum.EVENT_CALL_DENY,meetingSession.getRequesterId(),null);
                            meetingSessionEventToPeer = EventDTO.buildPersonal(EventEnum.EVENT_CALL_FAIL,meetingSession.getPeerId(),null);
                        }
                    }else{
                        // 如果已进入IN MEETING阶段，置为closed
                        meetingSession.setStatus(SessionStatusEnum.CLOSED.getCode());
                        if(hostId.equals(meetingSession.getRequesterId())){
                            meetingSessionEventToRequester = EventDTO.buildPersonal(EventEnum.EVENT_CALL_HANGUP,meetingSession.getRequesterId(),null);
                            meetingSessionEventToPeer = EventDTO.buildPersonal(EventEnum.EVENT_CALL_HANGUP,meetingSession.getPeerId(),null);
                ...
                if(meetingSession.getStatus()==SessionStatusEnum.IN_CONFIRM.getCode()){
                    ...
                    redisUtils.hSet(MEETING_SESSION,sessionId,meetingSession); // 更新Session
                    redisUtils.hSet(MEETING_USER_ACTIVITY ,String.valueOf(hostId),sessionId,MEETING_USER_ACTIVITY_TIME);
                    EventDTO<MeetingSession> meetingSessionEventToRequester = EventDTO.buildPersonal(EventEnum.EVENT_CALL_PRE_CONNECT,meetingSession.getRequesterId(),meetingSession);
                    sendPersonalEvent(meetingSession.getRequesterId(),meetingSessionEventToRequester);
//
//                    EventDTO<MeetingSession> meetingSessionEventToPeer = EventDTO.buildPersonal(EventEnum.EVENT_CALL_PRE_CONNECT,meetingSession.getPeerId(),meetingSession);
//                    sendPersonalEvent(meetingSession.getPeerId(),meetingSessionEventToPeer);
...
//                            meetingSession.addSignalingItem("peerIcecandidate",signalingMap.get("peerIcecandidate"));
                    ...
                    if(meetingSession.getSignalingMap()!=null&&meetingSession.getSignalingMap().containsKey("requesterDone")&&meetingSession.getSignalingMap().containsKey("peerDone")){
                        ...
                        EventDTO<MeetingSession> meetingSessionEventToRequester = EventDTO.buildPersonal(EventEnum.EVENT_CALL_CONNECTED,meetingSession.getRequesterId(),meetingSession);
                        sendPersonalEvent(meetingSession.getRequesterId(),meetingSessionEventToRequester);

                        EventDTO<MeetingSession> meetingSessionEventToPeer = EventDTO.buildPersonal(EventEnum.EVENT_CALL_CONNECTED,meetingSession.getPeerId(),meetingSession);
                        sendPersonalEvent(meetingSession.getPeerId(),meetingSessionEventToPeer);
                    }else{
                    ...
                    if(hostId.equals(meetingSession.getRequesterId())){
                        if(!redisUtils.hHasKey(MEETING_USER_ACTIVITY,String.valueOf(meetingSession.getPeerId()))){
                            // host是requester，但是peer不在线了
                            isHangup = true;
                        }
                    }else if(hostId.equals(meetingSession.getPeerId())){
                        if(!redisUtils.hHasKey(MEETING_USER_ACTIVITY,String.valueOf(meetingSession.getRequesterId()))){
                    ...
                    if(isHangup){
                        ...
                        meetingSession.setGmtMeetingEnd(DateUtils.getCurrentDateTime());
                        redisUtils.hSet(MEETING_SESSION,sessionId,meetingSession); // 更新Session
                        EventDTO<MeetingSession> meetingSessionEventToRequester = EventDTO.buildPersonal(EventEnum.EVENT_CALL_HANGUP,meetingSession.getRequesterId(),meetingSession);
                        sendPersonalEvent(meetingSession.getRequesterId(),meetingSessionEventToRequester);

                        EventDTO<MeetingSession> meetingSessionEventToPeer = EventDTO.buildPersonal(EventEnum.EVENT_CALL_HANGUP,meetingSession.getPeerId(),meetingSession);
                        sendPersonalEvent(meetingSession.getPeerId(),meetingSessionEventToPeer);
    ...
    public MeetingSession checkSessionStatusActive(Long hostId, String sessionId) throws ServiceInternalException{
        ...
        if(NumberHelper.isIDValid(hostId)&&!meetingSession.getRequesterId().equals(hostId)&&!meetingSession.getPeerId().equals(hostId)){
        ...
        if (!redisUtils.hHasKey(MEETING_USER_ACTIVITY,String.valueOf(meetingSession.getRequesterId()))) {    // 如果requester active不存在了，说了当前seesionId不存在或者过期了
            meetingSession.setStatus(SessionStatusEnum.FAIL.getCode());
            meetingSession.setGmtMeetingEnd(DateUtils.getCurrentDateTime());
            redisUtils.hSet(MEETING_SESSION,sessionId,meetingSession); // 更新Session

            EventDTO<MeetingSession> meetingSessionEventToRequester = EventDTO.buildPersonal(EventEnum.EVENT_CALL_FAIL,"你的当前Session不存在",meetingSession.getRequesterId(),meetingSession);
            sendPersonalEvent(meetingSession.getRequesterId(),meetingSessionEventToRequester);

            // 通知Requester，对方状态
            EventDTO<MeetingSession> meetingSessionEventToPeer = EventDTO.buildPersonal(EventEnum.EVENT_CALL_OFFLINE,meetingSession.getPeerId(),meetingSession);
            sendPersonalEvent(meetingSession.getPeerId(),meetingSessionEventToPeer);

            redisUtils.hDel(MEETING_USER_ACTIVITY ,meetingSession.getPeerId());
            throw new ServiceInternalException(ResultCode.MEETING_SESSION_GIVE_UP, "发起人放弃此Session");
        }else if (!redisUtils.hHasKey(MEETING_USER_ACTIVITY,String.valueOf(meetingSession.getPeerId()))) {    // 如果peer active不存在了，说了当前seesionId不存在或者过期了
            meetingSession.setStatus(SessionStatusEnum.FAIL.getCode());
            meetingSession.setGmtMeetingEnd(DateUtils.getCurrentDateTime());
            redisUtils.hSet(MEETING_SESSION,sessionId,meetingSession); // 更新Session

            // 通知Requester，对方状态
            EventDTO<MeetingSession> meetingSessionEventToRequester = EventDTO.buildPersonal(EventEnum.EVENT_CALL_OFFLINE,meetingSession.getRequesterId(),meetingSession);
            sendPersonalEvent(meetingSession.getRequesterId(),meetingSessionEventToRequester);
            redisUtils.hDel(MEETING_USER_ACTIVITY ,meetingSession.getRequesterId());

            // 通知Requester，对方状态
            EventDTO<MeetingSession> meetingSessionEventToPeer = EventDTO.buildPersonal(EventEnum.EVENT_CALL_FAIL,"你的当前Session不存在",meetingSession.getPeerId(),meetingSession);
            sendPersonalEvent(meetingSession.getPeerId(),meetingSessionEventToPeer);
            throw new ServiceInternalException(ResultCode.MEETING_SESSION_GIVE_UP,"接收人放弃此Session");
        }

        if (!sessionId.equals(redisUtils.hGet(MEETING_USER_ACTIVITY,String.valueOf(meetingSession.getRequesterId())))||!sessionId.equals(redisUtils.hGet(MEETING_USER_ACTIVITY,String.valueOf(meetingSession.getPeerId())))) {
            meetingSession.setStatus(SessionStatusEnum.FAIL.getCode());
            meetingSession.setGmtMeetingEnd(DateUtils.getCurrentDateTime());
            redisUtils.hSet(MEETING_SESSION,sessionId,meetingSession); // 更新Session

            // 如果双方active存在，但是双方存储的sessionId不是同一个sessionId，就说嘛对方忙
            EventDTO<MeetingSession> meetingSessionEventToHost = EventDTO.buildPersonal(EventEnum.EVENT_CALL_BUSY,meetingSession.getRequesterId(),meetingSession);
            sendPersonalEvent(hostId,meetingSessionEventToHost);
...
//        if(!UserActiveStatusEnum.getAllOnLineCodeList().contains(userService.getUserActiveStatus(meetingSession.getRequesterId()))){
        if(!UserActiveStatusEnum.ACTIVE_VISIBLE.getCode().equals(userService.getUserActiveStatus(meetingSession.getRequesterId()))){
            ...
            EventDTO<MeetingSession> meetingSessionEventToRequester = EventDTO.buildPersonal(EventEnum.EVENT_CALL_FAIL,meetingSession.getRequesterId(),meetingSession);
            sendPersonalEvent(meetingSession.getRequesterId(),meetingSessionEventToRequester);
...
//        if(!UserActiveStatusEnum.getAllOnLineCodeList().contains(userService.getUserActiveStatus(meetingSession.getPeerId()))){
        if(!UserActiveStatusEnum.ACTIVE_VISIBLE.getCode().equals(userService.getUserActiveStatus(meetingSession.getPeerId()))){
            ...
            EventDTO<MeetingSession> meetingSessionEventToPeer = EventDTO.buildPersonal(EventEnum.EVENT_CALL_FAIL,meetingSession.getPeerId(),meetingSession);
            sendPersonalEvent(meetingSession.getPeerId(),meetingSessionEventToPeer);
