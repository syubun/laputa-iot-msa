package com.laputa.iot.common.data.mybatis.conditions.query;
...
import com.laputa.iot.common.core.util.StrPool;
 ...
 * 查询构造器
 */
public class LaputaWrapper<T> extends AbstractLambdaWrapper<T, LaputaWrapper<T>>
        implements Query<LaputaWrapper<T>, T, SFunction<T, ?>> {
...
//            this.entity = (T) replace(entity);
     ...
     */
    private LaputaWrapper(T entity, Class<T> entityClass, SharedString sqlSelect, AtomicInteger paramNameSeq,
...
//        this.entityClass = entityClass;
    ...
    @SafeVarargs
    @Override
    public final LaputaWrapper<T> select(SFunction<T, ?>... columns) {
    ...
    @Override
    public LaputaWrapper<T> select(Predicate<TableFieldInfo> predicate) {
        return this.select(getEntityClass(), predicate);
    }
     ...
     */
    @Override
    public LaputaWrapper<T> select(Class<T> entityClass, Predicate<TableFieldInfo> predicate) {
//        this.entityClass = entityClass;
        setEntityClass(entityClass);
     ...
     */
    @Override
    protected LaputaWrapper<T> instance() {
        return new LaputaWrapper<>(getEntity(), getEntityClass(), null, this.paramNameSeq, this.paramNameValuePairs,
                new MergeSegments(), SharedString.emptyString(), SharedString.emptyString());
    }

    @Override
    public LaputaWrapper<T> eq(SFunction<T, ?> column, Object val) {
        return super.eq(this.checkCondition(val), column, val);
    }

    @Override
    public LaputaWrapper<T> ne(SFunction<T, ?> column, Object val) {
        return super.ne(this.checkCondition(val), column, val);
    }

    @Override
    public LaputaWrapper<T> gt(SFunction<T, ?> column, Object val) {
        return super.gt(this.checkCondition(val), column, val);
    }

    @Override
    public LaputaWrapper<T> ge(SFunction<T, ?> column, Object val) {
        return super.ge(this.checkCondition(val), column, val);
    }

    public LaputaWrapper<T> geHeader(SFunction<T, ?> column, LocalDateTime val) {
        if (val != null) {
    ...
    public LaputaWrapper<T> geHeader(SFunction<T, ?> column, LocalDate val) {
    ...
    @Override
    public LaputaWrapper<T> lt(SFunction<T, ?> column, Object val) {
        return super.lt(this.checkCondition(val), column, val);
    }

    @Override
    public LaputaWrapper<T> le(SFunction<T, ?> column, Object val) {
        return super.le(this.checkCondition(val), column, val);
    }

    public LaputaWrapper<T> leFooter(SFunction<T, ?> column, LocalDateTime val) {
        if (val != null) {
    ...
    public LaputaWrapper<T> leFooter(SFunction<T, ?> column, LocalDate val) {
        LocalDateTime dateTime = null;
        if (val != null) {
    ...
    @Override
    public LaputaWrapper<T> in(SFunction<T, ?> column, Collection<?> coll) {
        return super.in(coll != null && !coll.isEmpty(), column, coll);
    }

    @Override
    public LaputaWrapper<T> in(SFunction<T, ?> column, Object... values) {
    ...
    @Override
    public LaputaWrapper<T> like(SFunction<T, ?> column, Object val) {
    ...
    @Override
    public LaputaWrapper<T> notLike(SFunction<T, ?> column, Object val) {
     ...
     * @return
     */
    public LaputaWrapper<T> cancelSkipEmpty() {
        this.skipEmpty = false;
        return this;
    }

    @Override
    public LaputaWrapper<T> likeLeft(SFunction<T, ?> column, Object val) {
        return super.likeLeft(this.checkCondition(val), column, BaseLikeTypeHandler.likeConvert(val));
    }

    @Override
    public LaputaWrapper<T> likeRight(SFunction<T, ?> column, Object val) {
     ...
     * @return
     */
    public <A extends Object> LaputaWrapper<T> ignore(BiFunction<T, A, ?> setColumn) {
        setColumn.apply(getEntity(), null);
        return this;
    }
